...
Get-ChildItem -Recurse -File -Path "dist" -Filter "*windows-setup.exe" | Copy-Item -Destination "out\Tomato-Windows-Setup-x64.exe" -ErrorAction SilentlyContinue
Get-ChildItem -Recurse -File -Path "dist" -Filter "*windows.msix" | Copy-Item -Destination "out\Tomato-Windows-Setup-x64.msix" -ErrorAction SilentlyContinue
...
Compress-Archive -Force -Path "dist\tmp\hiddify-next" -DestinationPath "out\Tomato-Windows-Portable-x64.zip" -ErrorAction SilentlyContinue
