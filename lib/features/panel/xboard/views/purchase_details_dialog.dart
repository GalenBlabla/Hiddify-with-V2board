...
class _PurchaseDetailsDialogState extends ConsumerState<PurchaseDetailsDialog> {
  ...
  Future<void> _handleSubscribe() async {
    ...
    try {
      ...
      if (orderResponse != null && orderResponse['status'] == 'success') {
        ...
        final errorMessage = orderResponse?['message']?.toString();
  ...
  Future<void> _monitorOrderStatus() async {
    ...
    MonitorPayStatus().monitorOrderStatus(_tradeNo!, accessToken,
        (bool isPaid) {
      if (isPaid) {
        print('订单支付成功');
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _showPaymentMethodsDialog(List<dynamic> paymentMethods) async {
    ...
    if (orderDetails['status'] == 'success') {
      ...
      final totalAmount = (orderData['total_amount'] as num? ?? 0) / 100.0;
      final balanceAmount = (orderData['balance_amount'] as num? ?? 0) / 100.0;
                ...
                if (totalAmount <= 0)
                  ElevatedButton(
                    onPressed: () {
                      ...
                      _handlePayment({'id': 'wallet_balance'});
                    },
                  ...
                  ...paymentMethods.map((method) {
                    ...
                    final feePercent = double.tryParse(
